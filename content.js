...
// #########################################################################################
// ########################################################################################################
// Fonction pour charger dynamiquement les réglages du Popup
function chargerParametres(callback) {
    chrome.storage.local.get(['tomy_msgAuto', 'tomy_tempsMsg', 'tomy_nbrRepeat'], (data) => {
        if (data.tomy_msgAuto !== undefined) {
            // Sépare par des virgules si l'ami a mis plusieurs messages
            configor.messageAuto = data.tomy_msgAuto.split(',').map(m => m.trim());
        }
        if (data.tomy_tempsMsg !== undefined) messageAutoChaque = Number(data.tomy_tempsMsg);
        if (data.tomy_nbrRepeat !== undefined) nbrRepeat = Number(data.tomy_nbrRepeat);
...
chrome.storage.local.get(localostomios, (data) => {
    if (!chrome.runtime.lastError) {
        if (data.nbrRepeat !== undefined) nbrRepeat = data.nbrRepeat;
        if (data.timeReapeat !== undefined) timeReapeat = data.timeReapeat;
        if (data.messageAutoChaque !== undefined) messageAutoChaque = data.messageAutoChaque;
        if (data.seuilSpamDangereux !== undefined) configor.seuilSpamDangereux = data.seuilSpamDangereux;
        if (data.messageAuto !== undefined) configor.messageAuto = data.messageAuto;
        if (data.motsNiveau1 !== undefined) configor.motsNiveau1 = data.motsNiveau1;
        if (data.acceptInsult !== undefined) configor.acceptInsult = data.acceptInsult;
        if (data.motsNiveau3 !== undefined) configor.motsNiveau3 = data.motsNiveau3;
    }
});
...
function demarrerSurveillance() {
    ...
    if (!livedone) {
        surveillanceTimeout = setTimeout(demarrerSurveillance, 500);
    }
}
...
function arreterModoComplet() {
    ...
    livedone = true;
    removingNow = false;
    console.warn("🛑 Tous les scripts de modération ont été stoppés.");
}
